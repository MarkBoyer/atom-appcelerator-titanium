...
const Appc = {
	...
	/**
	* Returns if LiveView hook is defined
	*
	* @returns {Object}
	*/
	hasLiveViewHook() {
		const configPath = path.join(homedir(), '.titanium/config.json');
		var titaniumConfig = {};
		if (fs.existsSync(configPath)) {
			titaniumConfig = JSON.parse(fs.readFileSync(configPath, 'utf8'));
			for (const hook of titaniumConfig.paths.hooks) {
				if (hook.indexOf('com.appcelerator.titanium.liveview.core_') >= 0) {
					console.log("LiveView Hook Exists!");
					return true;
				}
			}
		}
		console.log("LiveView Hook DOES NOT Exist!");
		return false;
	}
};
